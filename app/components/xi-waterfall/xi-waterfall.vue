	...
	<view class="container">
		...
		<view class="column" id="columnFirst">
			<view class="item" v-for="(item, index) in columnFirst" :key="index">
				<image :src="item.ax[0]" mode="widthFix" lazy-load></image>
...
export default {
	...
	watch: {
		list(newVal, oldVal) {
			console.log(' water fall watch --oldVal->', JSON.stringify(oldVal));
			console.log(' water fall watch newVal>', JSON.stringify(newVal)); 
			//当监听到,数据有变化时
			if (newVal.length === 0) {
				//清空数据
				this.clear();
